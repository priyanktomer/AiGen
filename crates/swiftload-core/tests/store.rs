...
fn checkpoints_survive_and_stay_exact() {
    ...
    assert_eq!(
        got.completed_ranges, ranges,
        "the fragmented set must round-trip exactly"
    );
...
fn an_unclean_shutdown_triggers_the_paranoid_rewind() {
    ...
    let ranges =
        RangeSet::from_pairs([(0, 50 * 1024 * 1024), (100 * 1024 * 1024, 50 * 1024 * 1024)]);
    s.checkpoint(&d.id, &ranges, ranges.total()).unwrap();
    ...
    assert_eq!(
        ranges.total() - recovered.total(),
        2 * 1024 * 1024,
        "one megabyte per fragment"
    );
}
...
fn a_clean_shutdown_keeps_every_byte() {
    ...
    assert_eq!(
        recovered, ranges,
        "a clean stop must not cost the user any progress"
    );
}
...
fn swapping_a_url_preserves_every_byte_of_progress() {
    ...
    assert_eq!(
        got.completed_ranges, ranges,
        "progress must be untouched by a URL swap"
    );
...
fn url_history_is_redacted() {
    ...
    for e in &hist {
        for secret in ["SECRET", "deadbeefcafe", "AKIAEXAMPLE"] {
            assert!(
                !e.url_redacted.contains(secret),
                "leaked {secret} in {}",
                e.url_redacted
            );
        }
    ...
    assert!(
        hist[1].url_redacted.contains("X-Amz-Signature="),
        "parameter names should survive"
    );
...
fn url_history_records_what_was_preserved_at_each_swap() {
    ...
    s.checkpoint(
        &d.id,
        &RangeSet::from_pairs([(0, 6_500_000_000)]),
        6_500_000_000,
    )
    .unwrap();
    s.swap_url(
    ...
    let hist = s.url_history(&d.id).unwrap();
    assert_eq!(
        hist[1].bytes_done_at_swap, 6_500_000_000,
        "the evidence trail of what was saved"
    );
}
...
fn a_chain_of_swaps_keeps_one_download_and_one_history() {
    ...
    for i in 0..4 {
        s.checkpoint(
            &d.id,
            &RangeSet::from_pairs([(0, (i + 1) * 1_000_000)]),
            (i + 1) * 1_000_000,
        )
        .unwrap();
        s.swap_url(
            &d.id,
            &format!("https://x/y?t={i}"),
            "https://x/y",
            None,
            None,
            None,
    ...
    assert!(
        saved.windows(2).all(|w| w[1] >= w[0]),
        "preserved bytes must only grow: {saved:?}"
    );
}
...
fn duplicate_detection_finds_only_resumable_downloads() {
    ...
    s.set_status(&paused.id, DownloadStatus::Paused, None)
        .unwrap();

    let mut done = rec();
    done.identity_hint = "hint-abc".into();
    s.insert(&done).unwrap();
    s.set_status(&done.id, DownloadStatus::Completed, None)
        .unwrap();
    ...
    assert_eq!(
        found.len(),
        1,
        "a finished download is not something to resume"
    );
...
fn status_transitions_and_listing() {
    ...
    s.set_status(&d.id, DownloadStatus::Failed, Some("link expired"))
        .unwrap();
    ...
    assert!(
        got.status.is_resumable(),
        "a failed download can still be retried"
    );
...
fn host_profiles_are_learned_and_updated() {
    ...
    s.record_host_profile("cdn.example.com", 8, 20_000_000, false)
        .unwrap();
    let p = s.host_profile("cdn.example.com").unwrap().unwrap();
    assert_eq!(p.best_observed_conns, Some(8));
    assert_eq!(p.samples, 1);

    s.record_host_profile("cdn.example.com", 4, 18_000_000, true)
        .unwrap();
    let p = s.host_profile("cdn.example.com").unwrap().unwrap();
...
fn deleting_a_download_removes_its_history() {
    ...
    assert!(
        s.url_history(&d.id).unwrap().is_empty(),
        "history must cascade"
    );
}
