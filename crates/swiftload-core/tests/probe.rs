...
async fn detects_range_support_and_total_size_in_one_request() {
    ...
    assert!(
        h.stats().bytes_served <= 1024,
        "probe pulled {} bytes",
        h.stats().bytes_served
    );
}
...
async fn missing_content_length_yields_unknown_size() {
    ...
    assert_eq!(
        r.total_size, None,
        "no Content-Length means no size, not a guess"
    );
...
async fn a_transforming_content_encoding_disables_segmentation() {
    ...
    assert!(
        r.transforming_encoding,
        "server applied gzip despite our identity request"
    );
    assert_eq!(
        r.range_support,
        RangeSupport::Unsupported,
        "must refuse to segment"
    );
    assert!(!r.is_resumable());
    assert!(!r.is_segmentable());
}
...
async fn follows_redirect_chains_and_records_them() {
    ...
    assert!(
        r.final_url.path().contains("/plain/"),
        "final url {}",
        r.final_url
    );
}
...
async fn refuses_an_overlong_redirect_chain() {
    ...
    .unwrap_err();
    assert!(
        matches!(err, swiftload_core::task::probe::ProbeError::Redirect(_)),
        "{err}"
    );
}
...
async fn signed_urls_probe_before_and_fail_after_expiry() {
    ...
    let url = c
        .get(h.url("/mint/alpha?n=1048576"))
        .send()
        .await
        .unwrap()
        .text()
        .await
        .unwrap();
    let r = probe(&url, &settings(), &RequestSpec::default(), false)
        .await
        .unwrap();
    assert_eq!(r.total_size, Some(1_048_576));

    c.get(h.url("/expire/alpha")).send().await.unwrap();
    let err = probe(&url, &settings(), &RequestSpec::default(), true)
        .await
        .unwrap_err();
    assert!(
        matches!(
            err.class(),
            swiftload_core::http::errors::ErrorClass::UrlExpired
        ),
